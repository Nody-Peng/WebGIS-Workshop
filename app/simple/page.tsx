...
const tutorialSteps = [
  ...
  },
  {
    ...
    defaultJson: `{\n  "type": "FeatureCollection",\n  "features": [\n    {\n      "type": "Feature",\n      "properties": {\n        "id": "TP-001",\n        "district": "Wenshan",\n        "vulnerability_score": 2,\n        "needs": "Sandbags"\n      },\n      "geometry": { "type": "Point", "coordinates": [121.565, 24.985] }\n    }\n  ]\n}`,
    examples: [
      { label: "Safe household (score 2)", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    {\n      "type": "Feature",\n      "properties": { "id": "TP-001", "district": "Wenshan", "vulnerability_score": 2, "needs": "Sandbags" },\n      "geometry": { "type": "Point", "coordinates": [121.565, 24.985] }\n    }\n  ]\n}` },
      { label: "Critical household (score 5)", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    {\n      "type": "Feature",\n      "properties": { "id": "TP-001", "district": "Wenshan", "vulnerability_score": 5, "needs": "Evacuation" },\n      "geometry": { "type": "Point", "coordinates": [121.565, 24.985] }\n    }\n  ]\n}` },
      { label: "All four risk levels", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    { "type": "Feature", "properties": { "id": "A", "vulnerability_score": 1, "needs": "None" }, "geometry": { "type": "Point", "coordinates": [121.560, 24.982] } },\n    { "type": "Feature", "properties": { "id": "B", "vulnerability_score": 3, "needs": "Water" }, "geometry": { "type": "Point", "coordinates": [121.565, 24.985] } },\n    { "type": "Feature", "properties": { "id": "C", "vulnerability_score": 4, "needs": "Food" }, "geometry": { "type": "Point", "coordinates": [121.570, 24.988] } },\n    { "type": "Feature", "properties": { "id": "D", "vulnerability_score": 5, "needs": "Evacuation" }, "geometry": { "type": "Point", "coordinates": [121.575, 24.991] } }\n  ]\n}` },
    ],
    showTileSwitcher: false,
  },
  {
  ...
  },
  {
    ...
    examples: [
      { label: "Single flood zone", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    {\n      "type": "Feature",\n      "properties": { "name": "Flood Zone A", "risk": "High", "zone_type": "flood" },\n      "geometry": { "type": "Polygon", "coordinates": [[[121.558,24.980],[121.578,24.980],[121.578,24.996],[121.558,24.996],[121.558,24.980]]] }\n    }\n  ]\n}` },
      { label: "Flood zone + households inside", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    {\n      "type": "Feature",\n      "properties": { "name": "Flood Zone A", "risk": "High", "zone_type": "flood" },\n      "geometry": { "type": "Polygon", "coordinates": [[[121.558,24.980],[121.578,24.980],[121.578,24.996],[121.558,24.996],[121.558,24.980]]] }\n    },\n    { "type": "Feature", "properties": { "id": "TP-001", "vulnerability_score": 5, "needs": "Evacuation" }, "geometry": { "type": "Point", "coordinates": [121.565, 24.985] } },\n    { "type": "Feature", "properties": { "id": "TP-002", "vulnerability_score": 3, "needs": "Sandbags" }, "geometry": { "type": "Point", "coordinates": [121.570, 24.990] } },\n    { "type": "Feature", "properties": { "id": "TP-003", "vulnerability_score": 2, "needs": "None" }, "geometry": { "type": "Point", "coordinates": [121.562, 24.988] } }\n  ]\n}` },
      { label: "Two zones (flood + evacuation)", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    { "type": "Feature", "properties": { "name": "Flood Zone A", "risk": "High", "zone_type": "flood" }, "geometry": { "type": "Polygon", "coordinates": [[[121.558,24.980],[121.578,24.980],[121.578,24.996],[121.558,24.996],[121.558,24.980]]] } },\n    { "type": "Feature", "properties": { "name": "Evacuation Zone B", "risk": "Medium", "zone_type": "evacuation" }, "geometry": { "type": "Polygon", "coordinates": [[[121.578,24.980],[121.595,24.980],[121.595,24.996],[121.578,24.996],[121.578,24.980]]] } }\n  ]\n}` },
      { label: "Irregular polygon (real shape)", json: `{\n  "type": "FeatureCollection",\n  "features": [\n    {\n      "type": "Feature",\n      "properties": { "name": "Wenshan Flood Basin", "risk": "High", "zone_type": "flood" },\n      "geometry": { "type": "Polygon", "coordinates": [[[121.555,24.978],[121.568,24.975],[121.580,24.982],[121.576,24.995],[121.560,24.998],[121.550,24.990],[121.555,24.978]]] }\n    }\n  ]\n}` },
    ],
    showTileSwitcher: true,
  },
  {
  ...
  <script>
    ...
    const layer = L.geoJSON(data, {
      pointToLayer: function(feature, latlng) {
        ...
        return L.circleMarker(latlng, { radius: 12, fillColor: getColor(score), color: '#fff', weight: 2, fillOpacity: 1 });
      },
      ...
      onEachFeature: function(feature, layer) {
        const p = feature.properties;
        if (p.id) {
          layer.bindPopup('<b>' + p.id + '</b>' + (p.district ? '<br>District: ' + p.district : '') + (p.vulnerability_score !== undefined ? '<br>Score: ' + p.vulnerability_score : '') + (p.needs ? '<br>Needs: <b>' + p.needs + '</b>' : ''));
        } else if (p.name) {
...
// ─── Examples Drawer ───────────────────────────────────────────
function ExamplesDrawer({ examples, onLoad }: { examples: { label: string; json: string }[]; onLoad: (json: string) => void }) {
  ...
  return (
    <div className="border-t border-slate-800">
      <button onClick={() => setOpen(!open)}
        ...
        <span className="transition-transform duration-200 text-slate-500" style={{ transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>▾</span>
      </button>
      {open && (
        <div className="px-3 pb-3 space-y-1.5">
          {examples.map((ex) => (
            <button key={ex.label} onClick={() => { onLoad(ex.json); setOpen(false); }}
              ...
              <span className="text-xs text-slate-300 group-hover:text-white transition-colors">{ex.label}</span>
              <span className="float-right text-[10px] text-slate-600 group-hover:text-sky-400 transition-colors">Load →</span>
            </button>
          ))}
        </div>
      )}
        ...
        return (
          <div key={i}
            className={`flex-1 py-2.5 text-center text-[10px] font-bold uppercase tracking-widest border-r border-slate-800 last:border-r-0 transition-colors
              ${active ? "bg-sky-600 text-white" : done ? "bg-slate-800 text-sky-400" : "bg-slate-900 text-slate-600"}`}>
            {done ? "✓" : i + 1}
          </div>
        );
      })}
...
// ─── Tile Switcher ─────────────────────────────────────────────
function TileSwitcherOverlay({ current, onChange }: { current: TileKey; onChange: (k: TileKey) => void }) {
  return (
...
export default function SimpleTutorialPage() {
  ...
  const [activeGeoJson, setActiveGeoJson] = useState<any>(JSON.parse(tutorialSteps[0].defaultJson));
  ...
  const handleRunCode = () => {
    try {
      ...
      if (parsed.type !== "FeatureCollection") throw new Error("Must be a FeatureCollection.");
  ...
  const handleDownload = () => {
    ...
    a.href = url; a.download = "my-webgis-map.html"; a.click();
    ...
    <div className="h-screen bg-slate-950 text-slate-200 flex flex-col overflow-hidden">
      ...
      <header className="flex-none flex items-center justify-between px-6 h-12 bg-slate-900 border-b border-slate-800 z-20">
        <div className="flex items-center gap-4">
          <Link href="/"
            className="text-sm font-bold text-white hover:text-slate-300 transition-colors tracking-tight">
            WebGIS Workshop
          </Link>
          <span className="text-slate-700">/</span>
          <span className="hidden sm:block text-[11px] text-slate-500 uppercase tracking-widest font-semibold">
            Hands-on Practice
          </span>
        </div>
        <div className="flex items-center gap-4">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        ...
        <div className="w-full lg:w-[480px] flex-none flex flex-col bg-slate-900 border-r border-slate-800 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            <div className="p-6 space-y-5">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-800">
                  ...
                  <span className="ml-2 text-[10px] text-slate-500 font-mono uppercase tracking-widest">Core Concept</span>
                </div>
              ...
              <div className="border border-sky-500/20 bg-sky-500/5 rounded-xl p-4">
                <p className="text-[10px] font-bold text-sky-400 uppercase tracking-widest mb-2">Your Mission</p>
            ...
            <ExamplesDrawer examples={step.examples} onLoad={handleLoadExample} />
          </div>

          {/* ── JSON Editor ── */}
          <div className="flex-none border-t border-slate-800 bg-slate-950 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">GeoJSON Editor</span>
              {errorMsg && (
                <span className="text-[10px] font-semibold text-red-400 bg-red-400/10 px-2 py-1 rounded-md">{errorMsg}</span>
              )}
            </div>
            <textarea
              value={jsonInput}
